...
#define SOLAR      1
#define FORCED     2
#define STOPPED    3    //KEEP LAST ONE
...
void Manage_Heat()
{
  ...
  switch (Mode)
  {
    ...
    { 
        ...
        {
          if (DEBUG_MODE) Serial.println("AUTO rechauffe ");
          valueGrad=MAX_POWER;
        }
...
void SendMqtt()
{
    ...
    publishstr="";
    if (DEBUG_WIFI) Serial.println(tempWater);
    publishstr="{\"idx\":49, \"nvalue\": 0, \"svalue\" :\"";
    publishstr+=  TempECS + ";";
    publishstr+=  TempECS +"\"}\n";
    if (DEBUG_WIFI) Serial.println(publishstr);
    publishstr.toCharArray(publishArray, publishstr.length());
    if (DEBUG_DOSEND) client.publish("domoticz/in",publishArray); //"{\"idx\":48, \"nvalue\": 0, \"svalue\" :\"510;210\"}"); 
